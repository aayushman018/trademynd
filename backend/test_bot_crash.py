...
async def main():
    db = SessionLocal()
    try:
        # Need to find a connected user
        user = db.query(User).filter(User.telegram_connected.is_(True)).first()
        if not user:
            print("No connected user found.")
            return

        bot = BotService(db)
        # Mock what _handle_image_message tries to do on fallback
        try:
